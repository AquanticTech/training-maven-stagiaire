    ...
    for filename in os.listdir(directory):
        if filename.endswith(".md"):
            with open(os.path.join(directory, filename), 'r', encoding='utf-8') as file:
                ...
                toc += f"## [{filename[:-3]}]({filename[:-3]}.html)\n\n"
                for line in content:
                    if line.startswith("# "):
                        toc += f"- [{line[2:].strip()}]({filename[:-3]}.html#{line[2:].strip().replace(' ', '-')})\n"
                    elif line.startswith("## "):
                        toc += f"  - [{line[3:].strip()}]({filename[:-3]}.html#{line[3:].strip().replace(' ', '-')})\n"
                    elif line.startswith("### "):
                        toc += f"    - [{line[4:].strip()}]({filename[:-3]}.html#{line[4:].strip().replace(' ', '-')})\n"
                toc += "\n"
